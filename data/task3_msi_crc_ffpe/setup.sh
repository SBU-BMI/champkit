...
do_task3() (
    ...
    # Download Task 3 data (CRC FFPE)
    echo "[champkit] Downloading task 3 data..."
    wget $wget_args https://zenodo.org/record/2530835/files/CRC_DX_TEST_MSIMUT.zip?download=1
    wget $wget_args https://zenodo.org/record/2530835/files/CRC_DX_TEST_MSS.zip?download=1
    wget $wget_args https://zenodo.org/record/2530835/files/CRC_DX_TRAIN_MSIMUT.zip?download=1
    wget $wget_args https://zenodo.org/record/2530835/files/CRC_DX_TRAIN_MSS.zip?download=1
    ...
    unzip $unzip_args $DOWNLOAD_DIR/CRC_DX_TEST_MSIMUT.zip -d $IMAGE_DIR/test
    unzip $unzip_args $DOWNLOAD_DIR/CRC_DX_TEST_MSS.zip -d $IMAGE_DIR/test

    unzip $unzip_args $DOWNLOAD_DIR/CRC_DX_TRAIN_MSIMUT.zip -d $IMAGE_DIR/trainval
    unzip $unzip_args $DOWNLOAD_DIR/CRC_DX_TRAIN_MSS.zip -d $IMAGE_DIR/trainval
...
echo "[champkit] Seeing if I should download task 3"
...
if [ ! -f $here/trainval-partitions.csv ]; then
    ...
    (cd $here; python split-data.py $IMAGE_DIR/trainval)
fi
